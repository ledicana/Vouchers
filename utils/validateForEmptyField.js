...
export default function validateForEmptyField(objectToValidate) {
    let emptylocationId, emptybranchId, emptyname, emptydiscount, emptyremainingUses, emptyexpirationTime;
    ...
    for(let key in objectToValidate) {
        if(objectToValidate.hasOwnProperty(key)) {
           if(key === 'locationId') {
               ...
               emptyexpirationTime = (objectToValidate[key].length === 0 || objectToValidate[key].length <= 11) ? true : false;
           }
        }
